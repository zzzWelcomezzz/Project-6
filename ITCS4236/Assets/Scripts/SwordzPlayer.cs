	...
	void Update ()
	{
        ...
        */
        ...
        /*if(Input.GetButtonUp("Dodge_"+humanPlayer))
            ...
            else if (!Input.GetButton("Attack") && !swordzDude.status.running)
            {
				dude.Look();
        ...
        else if ((Input.GetButton("Dodge") && runbuttonTime != 0.0f &&
		...
		*/
        ...
        if((Input.GetButton("Block") && !Input.GetButton("Attack")))
		{
			if(!dude.blocking) {
				if(canLook)
				{
        ...
        if ((Input.GetButton("Block")) && canLook && mouseControls)
        {
            gameObject.SendMessage("LookAtMouse");
		}
